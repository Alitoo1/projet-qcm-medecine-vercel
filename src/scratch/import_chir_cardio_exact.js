...
async function runImportChirCardio() {
  console.log('--- IMPORT EXHAUSTIF DU SOUS-MODULE CHIRURGIE CARDIO-VASCULAIRE (MODULE 27 - SEMESTRE 5) ---');

  const sm = await prisma.sousModule.findFirst({
    where: { 
      moduleId: 27,
      nom: 'Chirurgie Cardio-vasculaire'
    },
    include: { cours: { orderBy: { ordre: 'asc' } } }
  });

  if (!sm) throw new Error('Sous-module Chirurgie Cardio-vasculaire dans Module 27 non trouvé !');
  ...
  const raw = fs.readFileSync('c:\\Users\\alita\\OneDrive\\Bureau\\alito-next\\extracted_chir_cardio_text.txt', 'utf8');
  const lines = raw.split('\n').map(l => l.replace(/\r$/, '').trim());

  let startIdx = 0;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes('1. Intro à la chirurgie Cardiaque') && i > 5) {
      startIdx = i;
      break;
    }
  }

  console.log(`Parsing Chirurgie Cardio-vasculaire from line ${startIdx} to ${lines.length}...`);
  ...
  console.log(`Parsed ${allQuestions.length} total questions for Chirurgie Cardio-vasculaire.`);

  let totalQcmInserted = 0;
  let totalQrInserted = 0;
  const courseCounts = { 1: { qcm: 0, qr: 0 }, 2: { qcm: 0, qr: 0 }, 3: { qcm: 0, qr: 0 }, 4: { qcm: 0, qr: 0 } };
  ...
  console.log('\n======================================================');
  console.log('✅ BILAN DÉTAILLÉ DU SOUS-MODULE CHIRURGIE CARDIO-VASCULAIRE :');
...
runImportChirCardio().finally(() => prisma.$disconnect());
