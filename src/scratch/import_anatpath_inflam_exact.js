...
async function runImportAnatPathInflam() {
  console.log('--- IMPORT EXACT DU SOUS-MODULE PATHOLOGIES INFLAMMATOIRES (ANATOMIE PATHOLOGIQUE) ---');
  ...
  if (!sm) throw new Error('Sous-Module 9 non trouvé !');

  const coursMap = {};
  for (const c of sm.cours) coursMap[c.ordre] = c.id;

  // Clear existing questions in SM 9
  ...
  const raw = fs.readFileSync('c:\\Users\\alita\\OneDrive\\Bureau\\alito-next\\extracted_anatpath_inflam_text.txt', 'utf8');
  const lines = raw.split('\n').map(l => l.replace(/\r$/, '').trim());

  let startIdx = 0;
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].includes("1. Anatomie pathologique : rôles et moyens")) {
      startIdx = i;
      break;
    }
  }

  console.log(`Parsing Pathologies Inflammatoires from line ${startIdx} to ${lines.length}...`);
  ...
  for (let i = startIdx; i < lines.length; i++) {
    ...
    // Strict Course headers: e.g. "1. Anatomie pathologique : rôles et moyens : (16 questions)"
    ...
    if (rawLine.startsWith('Pathologies Inflammatoires')) {
      continue;
    }
    ...
    if (currentCaseContext && currentQ && (
      rawLine.includes('A l\'examen histologique') ||
      rawLine.includes('A l’examen histologique') ||
      rawLine.includes('L\'examen histologique') ||
      rawLine.includes('L\'examen macroscopique') ||
      rawLine.includes('Lors de l\'exploration chirurgicale') ||
      rawLine.includes('Le pathologiste a reçu') ||
      rawLine.includes('A l\'ouverture, la muqueuse') ||
      rawLine.includes('Sur un foyer au niveau')
    )) {
      finalizeCurrentQ();
      currentCaseContext += '\n' + line;
      continue;
    ...
    if (qMatch && !isOption) {
      ...
      // In Course 6 (Cas cliniques généraux), all 56 questions are QR!
      if (currentCourseOrdre === 6) isQrType = true;
      // In Course 1, Q7-16 are QR
      if (currentCourseOrdre === 1 && parseInt(qMatch[1]) >= 7) isQrType = true;
      // In Course 2, Q4-15 are QR
      if (currentCourseOrdre === 2 && parseInt(qMatch[1]) >= 4) isQrType = true;
      // In Course 3, Q9-13 are QR
      if (currentCourseOrdre === 3 && parseInt(qMatch[1]) >= 9) isQrType = true;
      // In Course 4, Q6-8 are QR
      if (currentCourseOrdre === 4 && parseInt(qMatch[1]) >= 6) isQrType = true;
      // In Course 5, Q4-14 are QR
      if (currentCourseOrdre === 5 && parseInt(qMatch[1]) >= 4) isQrType = true;
  ...
  console.log(`Parsed ${allQuestions.length} total questions for Pathologies Inflammatoires.`);
  ...
  console.log('\n======================================================');
  console.log('✅ BILAN DÉTAILLÉ DE PATHOLOGIES INFLAMMATOIRES :');
  for (let ord = 1; ord <= 6; ord++) {
...
runImportAnatPathInflam().finally(() => prisma.$disconnect());
